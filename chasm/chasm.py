...
def print_c_array(out, f):
  print("// generated by chasm.py", file=f)
  print("static const int program[] = {", file=f)
  for ft in range(1, 3+1):
    for row in range(100):
      words = []
      for word_index in range(6):
        value = out.get(ft, row, word_index)
        words.append(f"{value.word}")
      words = ', '.join(words)
      print(f"  {words},", file=f)
  print("};", file=f)
