...
class Assembler(object):
  ...
  def patch_pulseamp(self, arg):
    ...
    # the names are quite permissive because they are generated by
    # a macro concatenating input and output program strings
    m = re.match(r"{pa-(?P<side>[ab])-(?P<name>.*)}", arg)
    if not m:
  ...
  def patch_argument(self, arg):
    patch_dispatch = {
        ...
        re.compile(r"{pa-[ab]-.*}"):      self.patch_pulseamp, # pulse amplifiers
